...
#if XCHAL_HAVE_HIFI4_VFPU || XCHAL_HAVE_FP || HAVE_HPFPU
...
inline_ int __feraiseexcept( int excepts )
{
    ...
    /* variant for single precision FPU */
  if ( 0 != ( excepts & ( FE_UNDERFLOW | FE_INEXACT | FE_INVALID | FE_OVERFLOW | FE_DIVBYZERO ) ) )
  {
    ...
    xtfloat unfl_tiny  = XT_CONST_S(1); XT_MOVT_S ( unfl_tiny, XT_AE_MOVXTFLOATX2_FROMINT32X2( AE_MOVI(1) ), b_unfl );
    xtfloat ovfl_bigf  = XT_CONST_S(1); XT_MOVT_S ( ovfl_bigf, XT_AE_MOVXTFLOATX2_FROMINT32X2( 0x7f7f7f7f ), b_ovfl ); /* 0x7f7f7f7f, 3.4e38f */
#elif XCHAL_HAVE_FP
    xtfloat unfl_tiny  = XT_CONST_S(1); XT_MOVT_S ( unfl_tiny, XT_WFR(XT_MOVI(1))       , b_unfl );
    xtfloat ovfl_bigf  = XT_CONST_S(1); XT_MOVT_S ( ovfl_bigf, XT_WFR(0x7f7f7f7f)       , b_ovfl ); /* 0x7f7f7f7f, 3.4e38f */
#endif
    int     inex_bigi  = XT_MOVI(1);    XT_MOVT   ( inex_bigi, 0x7f7f7f7f               , b_inex ); /* 0x7f7f7f7f */
    xtfloat inex_dum;
    ...
    __asm__ volatile ( "float.s %0, %1, 0" : "=v" (inex_dum ) : "v" (inex_bigi) ); /* (float)0x7f7f7f7f -> "inexact"         */
    __Pragma( "no_reorder" );
  }
#elif HAVE_HPFPU
    /* variant for half precision FPU */
  if ( 0 != ( excepts & ( FE_UNDERFLOW | FE_INEXACT | FE_INVALID | FE_OVERFLOW | FE_DIVBYZERO ) ) )
  {
    /* Most of the time there are no exceptions to raise. */
    __Pragma( "frequency_hint never" );
    /* Extract a flag for each of supported exceptions. */
    xtbool4 b_inv  = ( 0 != ( excepts & FE_INVALID   ) );
    xtbool4 b_divz = ( 0 != ( excepts & FE_DIVBYZERO ) );
    xtbool4 b_unfl = ( 0 != ( excepts & FE_UNDERFLOW ) );
    xtbool4 b_ovfl = ( 0 != ( excepts & FE_OVERFLOW  ) );
    xtbool4 b_inex = ( 0 != ( excepts & FE_INEXACT   ) );
    /* Auxiliary vars. */
    xthalfx4 inv_neg    = CONST_HX4(1); MOVT_HX4 ( inv_neg  , NEG_HX4( CONST_HX4(1) ), b_inv  );
    xthalfx4 divz_zero  = CONST_HX4(1); MOVT_HX4 ( divz_zero, CONST_HX4(0)              , b_divz );
    xthalfx4 unfl_tiny  = CONST_HX4(1); MOVT_HX4 ( unfl_tiny, AE_MOVXTHALFX4_FROMINT16X4(AE_MOVDA16(1))         , b_unfl );
    xthalfx4 ovfl_bigf  = CONST_HX4(1); MOVT_HX4 ( ovfl_bigf, AE_MOVXTHALFX4_FROMINT16X4(AE_MOVDA16(0x7b7b7b7b)), b_ovfl ); /* 0x7b7b, 61408 */
    ae_int16x4 inex_bigi  = XT_MOVI(1);  AE_MOVT16X4 ( inex_bigi, AE_MOVDA16(0x7f7f7f7f)                  , b_inex ); /* 0x7f7f7f7f */
    xthalfx4 inex_dum;
     ...
     *      in FCR is 1. */
    __Pragma( "no_reorder" );
    __asm__ volatile ( "rsqrt0.h %0, %0"   : "+v" (inv_neg  ) );                   /* sqrt(-1.f) -> "invalid"                */
    __asm__ volatile ( "recip0.h %0, %0"   : "+v" (divz_zero) );                   /* 1.f/0.f -> "division-by-zero"          */
    __asm__ volatile ( "mul.h %0, %0, %0"  : "+v" (unfl_tiny) );                   /* tiny*tine -> "underflow" and "inexact" */
    __asm__ volatile ( "add.h %0, %0, %0"  : "+v" (ovfl_bigf) );                   /* bigf+bigf -> "overflow" and "inexact"  */
    __asm__ volatile ( "float16.h %0, %1, 0" : "=v" (inex_dum ) : "v" (inex_bigi) ); /* (float)0x7f7f -> "inexact"         */
    __Pragma( "no_reorder" );
  }
