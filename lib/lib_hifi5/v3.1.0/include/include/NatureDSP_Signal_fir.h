...
/*===========================================================================
  ...
  ppa,pps                                 polyphase anaysis/synthesis
===========================================================================*/
...
/* Configure Filter Weights & States */
void bkfirf_coeffConfig( bkfirf_handle_t restrict _bkfir, const float32_t *h );
void bkfirf_stateConfig( bkfirf_handle_t restrict _bkfir, const float32_t *st );
...
#define FIR_CONVOLA32X16_SCRATCH_SIZE( N, M )    (((((N)+(M)-1)+1)&(~1))*4+(((M)+3)&(~3))*2)
...
#define FIR_CONVOLAF_SCRATCH_SIZE( N, M )        (  4*( (((N)+(M)-1+3) & ~3) + (((M)+3) & ~3)))
...
#define FIR_XCORRAF_SCRATCH_SIZE( N, M )          ( 4*( (((N)+(M)-1+3) & ~3) + (((M)+3) & ~3)) )
...
#define CXFIR_XCORRAF_SCRATCH_SIZE( N, M )        (8*(((((N) + (M) - 1) + 1)&~1) + (((M) + 1)&~1)))
#define FIR_XCORRA32X16_SCRATCH_SIZE( N, M )      (((((N)+(M)-1)+1)&(~1))*4+(((M)+3)&(~3))*2)
...
/*-------------------------------------------------------------------------
  ...
  Restrictions:
  x,y,r,s      should not overlap
  s            should be aligned on 16-byte boundary
  N,M          must be >0
  N>=M-1       minimum allowed length of vector x is the length of y minus one
-------------------------------------------------------------------------*/
...
#define FIR_ACORRAF_SCRATCH_SIZE( N )           FIR_XCORRAF_SCRATCH_SIZE(N,N)
...
/*-------------------------------------------------------------------------
  ...
  s       aligned on an 16-bytes boundary
-------------------------------------------------------------------------*/
#define FIR_LACORRA16X16_SCRATCH_SIZE( N )       ( 2*((N + 4 + 3)&~3) )
#define FIR_LACORRA32X32_SCRATCH_SIZE( N )       ( 4*(N + 5) )
...
/* Generic version */
void conv2d_gen_3x3_8x8   (void* pScr, int8_t*    z, const int8_t*    x, const int8_t*    y, int rsh, int P, int Q); // MxN=3x3
void conv2d_gen_5x5_8x8   (void* pScr, int8_t*    z, const int8_t*    x, const int8_t*    y, int rsh, int P, int Q); // MxN=5x5
void conv2d_gen_11x7_8x8  (void* pScr, int8_t*    z, const int8_t*    x, const int8_t*    y, int rsh, int P, int Q); // MxN=11x7
void conv2d_gen_3x3_8x16  (void* pScr, int16_t*   z, const int8_t*    x, const int16_t*   y, int rsh, int P, int Q); // MxN=3x3
void conv2d_gen_5x5_8x16  (void* pScr, int16_t*   z, const int8_t*    x, const int16_t*   y, int rsh, int P, int Q); // MxN=5x5
void conv2d_gen_11x7_8x16 (void* pScr, int16_t*   z, const int8_t*    x, const int16_t*   y, int rsh, int P, int Q); // MxN=11x7
void conv2d_gen_3x3_16x16 (void* pScr, int16_t*   z, const int16_t*   x, const int16_t*   y, int rsh, int P, int Q); // MxN=3x3
void conv2d_gen_5x5_16x16 (void* pScr, int16_t*   z, const int16_t*   x, const int16_t*   y, int rsh, int P, int Q); // MxN=5x5
void conv2d_gen_11x7_16x16(void* pScr, int16_t*   z, const int16_t*   x, const int16_t*   y, int rsh, int P, int Q); // MxN=11x7
void conv2d_gen_3x3f      (void* pScr, float32_t* z, const float32_t* x, const float32_t* y, int P, int Q); // MxN=3x3
void conv2d_gen_5x5f      (void* pScr, float32_t* z, const float32_t* x, const float32_t* y, int P, int Q); // MxN=5x5
void conv2d_gen_11x7f     (void* pScr, float32_t* z, const float32_t* x, const float32_t* y, int P, int Q); // MxN=11x7
void conv2d_gen_3x3_fp16  (void* pScr, float16_t* z, const float16_t* x, const float16_t* y, int P, int Q); // MxN=3x3
void conv2d_gen_5x5_fp16  (void* pScr, float16_t* z, const float16_t* x, const float16_t* y, int P, int Q); // MxN=5x5
void conv2d_gen_11x7_fp16 (void* pScr, float16_t* z, const float16_t* x, const float16_t* y, int P, int Q); // MxN=11x7

// scratch allocatation functions. return required scratch size in bytes
size_t conv2d_gen_3x3_8x8_getScratchSize    (int P, int Q); // MxN=3x3
size_t conv2d_gen_5x5_8x8_getScratchSize    (int P, int Q); // MxN=5x5
size_t conv2d_gen_11x7_8x8_getScratchSize   (int P, int Q); // MxN=11x7
size_t conv2d_gen_3x3_8x16_getScratchSize   (int P, int Q); // MxN=3x3
size_t conv2d_gen_5x5_8x16_getScratchSize   (int P, int Q); // MxN=5x5
size_t conv2d_gen_11x7_8x16_getScratchSize  (int P, int Q); // MxN=11x7
size_t conv2d_gen_3x3_16x16_getScratchSize  (int P, int Q); // MxN=3x3
size_t conv2d_gen_5x5_16x16_getScratchSize  (int P, int Q); // MxN=5x5
size_t conv2d_gen_11x7_16x16_getScratchSize (int P, int Q); // MxN=11x7
size_t conv2d_gen_3x3f_getScratchSize       (int P, int Q); // MxN=3x3
size_t conv2d_gen_5x5f_getScratchSize       (int P, int Q); // MxN=5x5
size_t conv2d_gen_11x7f_getScratchSize      (int P, int Q); // MxN=11x7
size_t conv2d_gen_3x3_fp16_getScratchSize   (int P, int Q); // MxN=3x3
size_t conv2d_gen_5x5_fp16_getScratchSize   (int P, int Q); // MxN=5x5
size_t conv2d_gen_11x7_fp16_getScratchSize  (int P, int Q); // MxN=11x7
...
void conv2d_3x3_fp16  (void* pScr, float16_t *z, const float16_t * x, const float16_t * y, int P, int Q); // MxN=3x3
void conv2d_5x5_fp16  (void* pScr, float16_t *z, const float16_t * x, const float16_t * y, int P, int Q); // MxN=5x5
void conv2d_11x7_fp16 (void* pScr, float16_t *z, const float16_t * x, const float16_t * y, int P, int Q); // MxN=11x7
...
size_t conv2d_3x3_fp16_getScratchSize  (int P, int Q); // MxN=3x3
size_t conv2d_5x5_fp16_getScratchSize  (int P, int Q); // MxN=5x5
size_t conv2d_11x7_fp16_getScratchSize (int P, int Q); // MxN=11x7
...
-------------------------------------------------------------------------*/
typedef void * rppafir32x32_handle_t;
typedef void * cppafir32x32_handle_t;
typedef void * rppafirf_handle_t;
typedef void * cppafirf_handle_t;

/* Returns: size of memory in bytes to be allocated */
size_t rppafir32x32_alloc   (int M, int N);
size_t cppafir32x32_alloc   (int M, int N);
size_t rppafirf_alloc       (int M, int N); 
...
rppafirf_handle_t     rppafirf_init      (void * objmem, int M, int N, const float32_t* h);
...
void rppafirf_process     (rppafirf_handle_t       handle, float32_t         * y, const float32_t        * x);
...
/*-------------------------------------------------------------------------
  ...
  M     32...640, multiplies of 32
  -------------------------------------------------------------------------*/
typedef void * rppsfir32x32_handle_t;
typedef void * cppsfir32x32_handle_t;
typedef void * rppsfirf_handle_t;
typedef void * cppsfirf_handle_t;
