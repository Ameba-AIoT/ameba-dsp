...
void IPC_INTUserHandler(IPC_TypeDef *IPCx, u8 IPC_Shiftbit, void *IrqHandler, void *IrqData)
{
	...
	if (IS_IPC_RX_CHNUM(IPC_Shiftbit)) {
		IPC_INTConfig(IPCx, IPC_Shiftbit, 1);
	}
}
...
//	IrqStatus = irq_disable_save();
	...
	if ((CPUID_idx != CPUID + 1) || (PXID_idx != PXID_Idx[SEM_Idx])) {
		DiagPrintf("CPU %d take sema %d for pxid %d fail: cpuid %d pxid %d has taken the sema\n", CPUID, SEM_Idx, PXID_Idx[SEM_Idx], CPUID_idx - 1, PXID_idx);
...
u32 IPC_SEMFree(IPC_SEM_IDX SEM_Idx)
{
	...
	if (CPUID_idx != (CPUID + 1)) {
		DiagPrintf("CPU %d free sema %d for pxid %d fail: cpuid %d, pxid %d is taking the sema\n", CPUID, SEM_Idx, PXID_Idx[SEM_Idx], CPUID_idx - 1, PXID_idx);
		return 0;
	}
