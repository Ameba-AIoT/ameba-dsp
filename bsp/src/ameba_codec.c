...
void AUDIO_CODEC_SetLDOMode(u32 ldo_mode)
{
	if (ldo_mode == LDO_POWER_ON) {
		...
		AUD->AUD_MICBST_CTL1 |= AUD_LDO_TUNE(14);
...
void AUDIO_CODEC_SetAmicInSE(u32 Amic_sel)
{
	...
	if (Amic_sel == AMIC1) {
		...
		AUD->AUD_MICBST_CTL0 &= ~(AUD_BIT_MICBST3_ENDF);
	}
}
...
void AUDIO_CODEC_SetMicBstGain(u32 Amic_sel, u32 gain)
{
	if (Amic_sel == AMIC1) {
		...
		AUD->AUD_MICBST_CTL0 &= ~(AUD_MASK_MICBST3_GSEL);
		AUD->AUD_MICBST_CTL0 |= AUD_MICBST3_GSEL(gain);
	}
}
