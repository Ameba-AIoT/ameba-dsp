...
GPIO_IRQ_FUN PortB_IrqHandler[32]; // The interrupt handler triggered by Port B[x]
void *PortA_IrqData[32];
void *PortB_IrqData[32];

GPIO_TypeDef *GPIO_PORTx[2] = {GPIOA_BASE, GPIOB_BASE};
GPIO_TypeDef *GPIO_PORTx_S[2] = {GPIOA_BASE_S, GPIOB_BASE_S};
...
/**
  ...
  * @retval None
  */
__weak void GPIO_INTMode(u32 GPIO_Pin, u32 NewState,
						 u32 GPIO_ITTrigger,
						 u32 GPIO_ITPolarity,
						 u32 GPIO_ITDebounce)
{
...
/**
  ...
  * @retval None
  */
__weak void GPIO_INTConfig(u32 GPIO_Pin, u32 NewState)
{
	GPIO_TypeDef *GPIO = NULL;
	u8 port_num;
	u8 pin_num;
...
/**
  ...
  * @retval None
  */
__weak u32 GPIO_INTHandler(void *pData)
{
		...
		for (i = 0; i < 20; i++) {
			if (IrqStatus & BIT(i)) {
				if (PortB_IrqHandler[i] != NULL) {
					if (port_a_in & BIT(i)) {
						event = HAL_IRQ_RISE | (i << 16) | BIT(21); /* (event | port_pin << 16 */
					} else {
						event = HAL_IRQ_FALL | (i << 16) | BIT(21); /* (event | port_pin << 16 */
					}
...
/**
  ...
  * @retval None
  */
__weak void GPIO_Direction(u32 GPIO_Pin, u32 data_direction)
{
	GPIO_TypeDef *GPIO = NULL;
	u8 port_num;
	u8 pin_num;
...
/**
  ...
  * @retval None
  */
__weak void GPIO_Init(GPIO_InitTypeDef  *GPIO_InitStruct)
{
	...
	Pinmux_Config((u8)GPIO_InitStruct->GPIO_Pin, PINMUX_FUNCTION_GPIO);
	...
	if (GPIO_InitStruct->GPIO_Mode == GPIO_Mode_INT) {
		...
		if (GPIO_InitStruct->GPIO_Mode == GPIO_Mode_OUT) {
			...
			GPIO_Direction(GPIO_InitStruct->GPIO_Pin, GPIO_Mode_IN);
			PAD_PullCtrl((u8)GPIO_InitStruct->GPIO_Pin, (u8)GPIO_InitStruct->GPIO_PuPd);
		}
	}
}
...
/**
  ...
  * @retval None
  */
__weak void GPIO_DeInit(u32 GPIO_Pin)
{
	GPIO_TypeDef *GPIO = NULL;
	u8 port_num;
	u8 pin_num;
	...
	RegValue &= ~(1 << pin_num);
	GPIO->PORT[0].GPIO_DDR = RegValue;
	PAD_PullCtrl((u8)GPIO_Pin, GPIO_PuPd_NOPULL);
...
/**
  ...
  */
__weak u32 GPIO_ReadDataBit(u32 GPIO_Pin)
{
	GPIO_TypeDef *GPIO = NULL;
	u8 port_num;
	u8 pin_num;
...
/**
  ...
  * @retval None
  */
__weak void GPIO_WriteBit(u32 GPIO_Pin, u32 Pin_State)
{
	GPIO_TypeDef *GPIO = NULL;
	u8 port_num;
	u8 pin_num;
...
/**
  ...
  * @retval None
  */
__weak void GPIO_PortDirection(u32 GPIO_Port, u32 GPIO_Mask, u32 data_direction)
{
	GPIO_TypeDef *GPIO = NULL;
	u32 RegValue;
...
/**
  ...
  */
__weak u32 GPIO_PortRead(u32 GPIO_Port, u32 GPIO_Mask)
{
	GPIO_TypeDef *GPIO = NULL;
	u32 RegValue;
...
/**
  ...
  * @retval None
  */
__weak void GPIO_PortWrite(u32 GPIO_Port, u32 GPIO_Mask, u32 Port_State)
{
	GPIO_TypeDef *GPIO = NULL;
	u32 RegValue;
...
/**
  ...
  * @retval None
  */
__weak void GPIO_DebounceClock(u32 GPIO_Port, u32 DivideCount)
{
	GPIO_TypeDef *GPIO = NULL;
...
/**
  ...
  * @retval None
  */
__weak void GPIO_LevelSync(u32 GPIO_Port, u32 NewState)
{
	GPIO_TypeDef *GPIO = NULL;
...
/**
  ...
  * @retval None
  */
__weak void GPIO_UserRegIrq(u32 GPIO_Pin, void *IrqHandler, void *IrqData)
{
	u8 port_num;
	u8 pin_num;
