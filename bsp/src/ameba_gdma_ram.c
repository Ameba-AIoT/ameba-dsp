...
static u8 GDMA_IrqNum[4] = {
	GDMA0_CHANNEL0_IRQ_DSP,
	GDMA0_CHANNEL1_IRQ_DSP,
	GDMA0_CHANNEL2_IRQ_DSP,
	GDMA0_CHANNEL3_IRQ_DSP
};
...
void GDMA_StructInit(PGDMA_InitTypeDef GDMA_InitStruct)
{
	...
	GDMA_InitStruct->SecureTransfer = 0x0;
}
...
void GDMA_Init(u8 GDMA_Index, u8 GDMA_ChNum, PGDMA_InitTypeDef GDMA_InitStruct)
{
	...
	CfgxLow = (GDMA_InitStruct->GDMA_ReloadSrc << 30) |
			  ...
			  ((u32)GDMA_InitStruct->GDMA_ChPrior << 4);
...
/**
  ...
  */
__weak  u8
GDMA_ChnlAlloc(u8 GDMA_Index, IRQ_FUN IrqFun, u32 IrqData, u32 IrqPriority)
{
	u32 found = 0;
	u8 GDMA_ChNum = 0;
	u8 IrqNum = 0;
...
{
	u8 IrqNum = GDMA0_CHANNEL0_IRQ_DSP;
	...
	//xtos_set_interrupt_handler(IrqNum, NULL, NULL, NULL);
	//xtos_interrupt_disable(IrqNum);

	//xt_set_interrupt_handler(IrqNum, NULL, NULL);
	//xt_ints_off(IrqNum);
